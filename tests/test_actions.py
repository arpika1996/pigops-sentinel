...
ID_PATTERNS = [r"\bh[12]-t\d", r"-v\d+\b", r"demo-farm", r"\btask-[a-z]", r"demo-(admin|worker)", r"@demo\.pigops"]
...
def test_spike_creates_a_task_for_the_farm_admin_and_notifies(actor, repo, cands, frozen_clock, notifier):
    c = cands[("MORTALITY_SPIKE", "H2 / 5. terem")]
    ...
    assert task.assignee_email == repo.users[ADMIN_UID].email          # farm-level admin = Kovács Péter
    assert task.sentinel_run_id == "run-1" and not task.done
    assert out.assignee_name == "Kovács Péter" and out.assignee_note is None
    ...
    assert out.notifications and out.notifications[0].to == "Kovács Péter" and out.notifications[0].status == SKIPPED
    ...
    assert log["assigneeName"] == "Kovács Péter" and "assigneeNote" not in log and log["notifications"][0]["status"] == "skipped"
...
def test_medium_and_low_severity_get_the_end_of_the_day(actor, repo, cands, frozen_clock):
    c = cands[("VALVE_INSTABILITY", "H1 / 4. terem")]
...
def test_task_notification_is_sent_when_the_admin_has_a_device(actor, repo, snapshot, cands, notifier):
    ...
    c = cands[("SILENT_ROOM", "H2 / 6. terem")]
    out = actor.act(_act(c, "high", "Walk through H2 / 6. terem", "Nobody has looked in for 3 days."))
    ...
    assert uid == ADMIN_UID and title == "Walk through H2 / 6. terem" and body.startswith("Nobody")
    assert data["source"] == "sentinel" and data["signalType"] == "SILENT_ROOM" and data["roomName"] == "H2 / 6. terem"
...
def test_second_task_for_the_same_room_and_signal_is_deduplicated(actor, repo, cands):
    ...
    assert "H2 / 5. terem" in second.reason and "open Sentinel task" in second.reason and "'First'" in second.reason
...
def test_per_run_task_cap(repo, snapshot, cands, notifier):
    ...
    a = actor.act(_act(cands[("MORTALITY_SPIKE", "H2 / 5. terem")]))
    b = actor.act(_act(cands[("SILENT_ROOM", "H2 / 6. terem")]))
...
def test_no_responsible_person_creates_an_unassigned_task_and_says_so(repo, snapshot, cands, cfg, notifier):
    ...
    out = actor.act(_act(cands[("MORTALITY_SPIKE", "H2 / 5. terem")]))
...
def test_deadline_risk_notifies_assignee_and_manager_and_stamps_the_task(actor, repo, cands, frozen_clock, notifier):
    c = cands[("DEADLINE_RISK", "H1 / 2. terem")]        # assigned to Nagy László (worker), overdue
    ...
    assert out.task_title == c.task.title and out.assignee_name == "Nagy László"
    assert [n.to for n in out.notifications] == ["Nagy László (assignee)", "Kovács Péter (farm manager)"]
...
def test_deadline_notifications_are_sent_with_devices(actor, repo, snapshot, cands, notifier):
    ...
    c = cands[("DEADLINE_RISK", "H1 / 2. terem")]
...
def test_partial_delivery_counts_as_done(repo, snapshot, cands, cfg):
    ...
    out = actor.act(_act(cands[("DEADLINE_RISK", "H1 / 2. terem")]))
...
def test_assignee_who_is_the_manager_is_notified_once(actor, repo, snapshot, cands):
    ...
    out = actor.act(_act(c))
    assert [n.to for n in out.notifications] == ["Kovács Péter (assignee)"]
...
def test_unknown_assignee_falls_back_to_the_manager(actor, repo, cands):
    ...
    repo.tasks[c.task.id] = c.task
    out = actor.act(_act(c))
    assert [n.to for n in out.notifications] == ["Kovács Péter (farm manager)"]
...
def test_non_act_decisions_do_nothing(actor, cands):
    c = cands[("MORTALITY_SPIKE", "H1 / 3. terem")]
...
def test_partition_handled_after_a_run(actor, repo, snapshot, cands, cfg):
    ...
    actor.act(_act(cands[("MORTALITY_SPIKE", "H2 / 5. terem")]))          # new Sentinel task
    actor.act(_act(cands[("DEADLINE_RISK", "H1 / 2. terem")]))            # notified (skipped, but stamped)
    ...
    assert set(handled_keys) == {("MORTALITY_SPIKE", "H2 / 5. terem"), ("DEADLINE_RISK", "H1 / 2. terem")}
    assert "open Sentinel task" in handled_keys[("MORTALITY_SPIKE", "H2 / 5. terem")]
    assert "already notified" in handled_keys[("DEADLINE_RISK", "H1 / 2. terem")]
    assert {(c.signal_type, c.room_name) for c in fresh} == {
        ("MORTALITY_SPIKE", "H1 / 3. terem"), ("DEADLINE_RISK", "H2 / 3. terem"),
        ("SILENT_ROOM", "H2 / 6. terem"), ("VALVE_INSTABILITY", "H1 / 4. terem"),
    }
